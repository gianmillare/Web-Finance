...
def history():
    ...
    transactions = db.execute("""
        SELECT symbol, shares, price, transacted FROM history
        WHERE user_id=?;
    """, session["user_id"])
...
def add_cash():
    if request.method == "GET":
        ...
        if cash_to_add == "" or cash_to_add == 0:
            return apology("please provide a valid amount to add.", 403)
        db.execute("UPDATE users SET cash = cash + ? WHERE id=?", (cash_to_add, session["user_id"]))
        db.execute("""
            INSERT INTO history (user_id, symbol, shares, price)
            VALUES (?, ?, ?, ?);
        """, (session["user_id"], "Added Cash", "---", cash_to_add))
